...
def parse_args():
    ...
    # 🚀 NEW: Add gradient accumulation parameter
...
def main():
    ...
    # 🚀 NEW: Print gradient accumulation info
    if rank == 0:
        effective_batch_size = args.batch_size * args.accumulation_steps * world_size
        print(f"🔍 Gradient Accumulation Setup:")
        print(f"   Batch size per GPU: {args.batch_size}")
        print(f"   Accumulation steps: {args.accumulation_steps}")
        print(f"   World size (GPUs): {world_size}")
        print(f"   Effective batch size: {effective_batch_size}")
        print(f"   Memory usage: Same as batch_size={args.batch_size}")
    ...
    # 🚀 NEW: Initialize gradient accumulation
    accumulation_steps = args.accumulation_steps
    
    # TODO: Training loop with gradient accumulation
    for epoch in range(args.epochs):
        sampler.set_epoch(epoch)
        
        total_loss = 0
        num_batches = 0
        accumulated_loss = 0  # 🚀 NEW: Track accumulated loss for logging
        
        for batch_idx, (data, target) in enumerate(dataloader):
            # TODO: Move data to device
            data, target = data.to(device), target.to(device)
            
            # 🚀 REMOVED: optimizer.zero_grad() from here (moved below)
            ...
            # TODO: Backward pass (accumulate gradients)
            scaled_loss.backward()  # This ACCUMULATES gradients instead of replacing them
            
            # 🚀 NEW: Track loss for logging (unscaled)
            accumulated_loss += loss.item()
            total_loss += loss.item()
            num_batches += 1
            
            # 🚀 NEW: Only update parameters every accumulation_steps
            if (batch_idx + 1) % accumulation_steps == 0:
                # Gradient clipping (optional) - applied to accumulated gradients
                ...
                # 🚀 NEW: Log accumulated loss (every accumulation cycle)
                if batch_idx % (10 * accumulation_steps) == (accumulation_steps - 1) and rank == 0:
                    avg_accumulated_loss = accumulated_loss / accumulation_steps
                    print(f'Epoch: {epoch}, Batch: {batch_idx - accumulation_steps + 1}-{batch_idx}, '
                          f'Avg Loss: {avg_accumulated_loss:.4f}')
                
                accumulated_loss = 0  # Reset for next accumulation cycle
            
            # 🚀 NEW: Alternative logging every 10 mini-batches (regardless of accumulation)
            elif batch_idx % 10 == 0 and rank == 0:
                print(f'Epoch: {epoch}, Mini-batch: {batch_idx}, Loss: {loss.item():.4f} '
                      f'(step {(batch_idx % accumulation_steps) + 1}/{accumulation_steps})')
        
        # 🚀 NEW: Handle remaining accumulated gradients at epoch end
        if num_batches % accumulation_steps != 0:
            ...
            if rank == 0:
                ...
                print(f'Applied final {remaining_steps} accumulated gradients at epoch end')
        ...
        if rank == 0:
            ...
            print(f'Parameter updates this epoch: {(num_batches + accumulation_steps - 1) // accumulation_steps}')
